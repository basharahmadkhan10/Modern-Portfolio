...
const projects = [
 ...
 desc: 'Multi-agent LangGraph system mimicking institutional equity research. A Bull agent and Bear agent conduct independent adversarial debates over live Yahoo Finance data, cross-examined by a CIO Judge Agent to issue INVEST/WATCH/PASS verdicts with kill-criteria. Features immutable evidence ledgers.',
 ...
 desc: 'Highly scalable competitive coding arena. Supports live matchmaking, synchronized WebSockets, strict anti-cheat (tab switch, fullscreen enforcement), Elo-based ranking, and live code execution via JDoodle against hidden test cases. Features a custom Monaco Editor implementation.',
 ...
 desc: 'Backend-focused civic issue management system utilizing a strict state-machine-driven lifecycle (Reported -> Verified -> Assigned -> Resolved). Enforces rigorous role-based access control (RBAC) and immutable audit-logging for complete transparency and accountability.',
 ...
 desc: 'A secure, gamified blood donation network. Features a privacy-first directory, real-time emergency broadcasts via $centerSphere MongoDB geo-queries (50km radius), concurrency control to prevent multiple donors, and a medical cooldown system to ensure donor safety.',
 ...
 desc: 'Production-ready Next.js 14 app for complex shared expenses. Implements time-travel membership boundaries (join/leave date constraints), transparent multi-currency conversions, anomaly review queues for messy CSV imports, and an optimal debt settlement engine.',
 ...
 desc: 'AI-powered platform parsing resumes against job descriptions to identify skill gaps and generate personalized study plans. Features precise match scoring, technical/behavioral question generation via Google Gemini API, secure JWT Auth, rate-limiting, and an elegant React interface.',
 ...
 desc: 'A full-stack invoice management system automating customer billing and reporting. Engineered dynamic invoice generation with printable formats, XML downloads, and automated monthly Excel reporting. Built on an Express.js backend with structured MongoDB data modeling.',
...
function ProjectCard({ project, index }) {
 ...
 >
 <div className="absolute inset-0 backface-hidden overflow-hidden bg-transparent rounded-2xl">
 <img
 src={project.image}
 alt={project.title}
 ...
 />
 <div className="absolute inset-x-0 bottom-8 z-10 flex flex-col items-center justify-center pointer-events-none">
    <p className="text-xs font-sans font-bold text-white uppercase tracking-widest mt-2 drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]">
      Click to Inspect
    </p>
  </div>
 </div>

 <div className="absolute inset-0 backface-hidden bg-white/30 dark:bg-black/40 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.1)] p-4 md:p-10 flex flex-col justify-between overflow-y-auto rounded-2xl" style={{ transform: 'rotateY(180deg)' }}>
 <div>
